//
// Created by lukemartinlogan on 2/12/24.
//
...
class Server : public TaskLib {
 ...
 public:
  ...
  void PutBlob(PutBlobTask *task, RunContext &rctx) {
    ...
    hermes::traits::Client TraitExec;
    TraitExec.Init(trait_id, HRUN_ADMIN->queue_id_);

    // Encode the blob update and store
    LPointer<hermes::traits::EncodeTask> encode_task =
        TraitExec.AsyncEncode(task->task_node_ + 1,
                              DomainId::GetLocal(),
                              blob_info, task, this);
    encode_task->Wait<TASK_YIELD_CO>(task);
    HRUN_CLIENT->DelTask(encode_task);
  ...
  void GetBlob(GetBlobTask *task, RunContext &rctx) {
    ...
    // Encode the blob update and store
    LPointer<hermes::traits::DecodeTask> decode_task =
        TraitExec.AsyncDecode(task->task_node_ + 1,
                              DomainId::GetLocal(),
                              blob_info, task, this);
    decode_task->Wait<TASK_YIELD_CO>(task);
    HRUN_CLIENT->DelTask(decode_task);
...
#endif  // HERMES_TASKS_HERMES_BLOB_MDM_SRC_HERMES_BLOB_MDM_SERVER_H_
